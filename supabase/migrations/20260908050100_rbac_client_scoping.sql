-- Phase 4: harden every existing table's RLS for the client role. Two
-- rules apply everywhere: (1) a client-role member can never write
-- anything (read-only, full stop), (2) a client-role member can only
-- read the handful of tables relevant to their own account — everything
-- internal (pipeline, delivery detail, sales-ops activity, config) is
-- invisible to them, not just hidden in the UI.
--
-- ALTER POLICY restates the whole USING/WITH CHECK clause; each block
-- below is the original clause from its origin migration with
-- "AND NOT is_client_role(...)" appended, or replaced with
-- can_view_account(...) where the table needs per-row account scoping.

-- ===== Internal-only tables: client gets zero access =====
...
-- ===== Own-account tables: client reads only their own account's rows =====
...
-- project_phases has no account_id of its own — scope via its project.
...
-- ===== Roster visibility: a client sees only their own membership row =====
