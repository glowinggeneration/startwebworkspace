...
$$;
-- Referenced directly inside RLS policies (see the next migration) — same
-- authenticated-must-have-EXECUTE reasoning as is_workspace_member/
-- has_workspace_role above.
...
as $$
  ...
  where user_id = _user_id and workspace_id = _workspace_id
$$;
-- Only ever called from inside can_view_account() below (another
-- SECURITY DEFINER function, already running elevated) — never referenced
-- directly by a policy or client code, so no authenticated grant is
-- needed here, unlike is_client_role/can_view_account.
revoke execute on function public.client_account_id_for(uuid, uuid) from public, anon, authenticated;

-- True for a non-client member of the workspace, or a client member
-- scoped to exactly this account. Every "own account" SELECT policy
-- added in the next migration reduces to this one check.
...
$$;
-- Referenced directly inside RLS policies (see the next migration) —
-- needs the authenticated grant, same as is_workspace_member/is_client_role.
...
-- The invite-accept page needs to show "you've been invited to X" before
-- the visitor has an account — a capability token (an unguessable uuid)
-- stands in for auth here, same trust model as a password-reset link.
...
-- Anyone holding the token can read that one row (not the workspace's
-- other invitations — there's no policy granting a bare workspace_id
-- scan), so the invite-accept page can render before sign-in.
...
-- Lets the (possibly signed-out) invite-accept page render "you've been
-- invited to <workspace> as <role>" without granting anon/authenticated
-- any broader read access to the workspaces or workspace_invitations
-- tables themselves — the token is the only thing that unlocks this.
