...
function OceanMesh({
  ...
  return (
    <group rotation={[(-90 * Math.PI) / 180, 0, 0]}>
      ...
      {showWireframe && <mesh ref={wireRef} geometry={geometry} material={wireframeMaterial} />}
    </group>
  );
}
...
function SceneContent({
  ...
  return (
    <>
      ...
      <rectAreaLight ref={rectLightRef} args={[accentColor, 20, 3, 3]} position={[2, 2, -20]} />
...
export function LiquidOcean({
  ...
  return (
    ...
    >
      ...
      {/* Overlay Content */}
      {children && (
        <div className="absolute inset-0 pointer-events-none select-none">{children}</div>
      )}
    </div>
  );
}
