...
export function PixelatedImageTrail({
  ...
  useEffect(() => {
    ...
    const createTrailImage = () => {
      ...
      const imageSource =
        validImagesRef.current[currentImageIndexRef.current % validImagesRef.current.length];
      ...
      const targetX =
        startX + (pointerPosRef.current.x - interpolatedPointerPosRef.current.x) * 0.45;
      const targetY =
        startY + (pointerPosRef.current.y - interpolatedPointerPosRef.current.y) * 0.45;
      ...
      if (pointerActiveRef.current) {
        interpolatedPointerPosRef.current = {
          x:
            ...
            (pointerPosRef.current.x - interpolatedPointerPosRef.current.x) * safeSmoothing,
          y:
            interpolatedPointerPosRef.current.y +
            (pointerPosRef.current.y - interpolatedPointerPosRef.current.y) * safeSmoothing,
        };
