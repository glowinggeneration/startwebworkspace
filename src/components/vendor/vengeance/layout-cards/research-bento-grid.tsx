...
export interface ResearchBentoGridProps extends Omit<
...
const DEFAULT_COPY: ResearchBentoGridCopy = {
  ...
  showcaseDescription:
    "From product UI to production code, ship focused improvements with the tools your team already trusts.",
  pricingTitle: (
    <>
      Senior execution.
      <br />
  ...
  pricingDescription:
    "Skip expanding headcount for every sprint. Get consistent product and engineering support at a predictable monthly cost.",
  pauseTitle: (
    <>
      Build on your terms.
      <br />
  ...
  activeDescription:
    "Between releases? Pause your workspace and resume when the next sprint is ready.",
...
function FeatureCopy({
  ...
  return (
    <div className={cn("absolute inset-x-0 bottom-0 z-20 px-5 pb-5 sm:px-7 sm:pb-7", className)}>
      <h3 className="text-[16px] font-medium leading-[1.08] tracking-[-0.045em] text-zinc-950 sm:text-[18px] dark:text-[#f1f1f1]">
        {title}
      </h3>
      <p className="mt-2.5 max-w-[330px] text-[12px] leading-[1.35] tracking-[-0.015em] text-zinc-600 sm:mt-3 sm:text-[13px] dark:text-[#858585]">
        {children}
      </p>
    </div>
  );
...
function DesignsPanel({
  ...
  const selected = Math.min(
    Math.max(isControlled ? selectedBrand : internalSelected, 0),
    brands.length - 1,
  );
  const cursorStops = brands.map(
    (_, index) => `${15 + (70 * index) / Math.max(brands.length - 1, 1)}%`,
  );
        ...
        <AnimatePresence initial={false} mode="wait">
          ...
          >
            ...
            <div className="mt-2 flex items-baseline gap-2 whitespace-nowrap">
              <span
                className={cn(
                  "text-[26px] font-medium tracking-[-0.055em] @min-[520px]:text-[30px]",
                  invoice.accent
                    ? "text-[#a4b000] dark:text-[#edff00]"
              ...
              {invoice.previousPrice && (
                <span className="text-[14px] text-zinc-500 line-through @min-[520px]:text-[16px] dark:text-[#767676]">
      ...
      <div className="absolute inset-x-0 top-0 flex h-[66%] items-center justify-center">
        ...
        >
          <motion.span
            ...
            transition={{
              duration: arrowLit ? 0.64 : 0.76,
              ease: arrowLit ? [0.16, 1, 0.3, 1] : [0.22, 1, 0.36, 1],
            }}
          />
