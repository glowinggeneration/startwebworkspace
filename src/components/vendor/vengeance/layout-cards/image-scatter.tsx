...
export function ImageScatter({
  ...
  useEffect(() => {
    if (!containerRef.current || !galleryRef.current || !headingRef.current || data.length === 0)
      return;
    ...
    const viewport = {
      ...
      rangeMin:
        Math.min(containerRef.current.clientWidth, containerRef.current.clientHeight) * 0.35,
    ...
    function updateViewport() {
      ...
      viewport.rangeMin =
        Math.min(containerRef.current.clientWidth, containerRef.current.clientHeight) * 0.35;
      viewport.rangeMax =
        Math.min(containerRef.current.clientWidth, containerRef.current.clientHeight) * 0.7;
    }
    ...
    function createCards(sectionIndex: number) {
      ...
      sectionData.images.forEach((src) => {
        ...
        card.className =
          "absolute rounded-2xl border-8 border-white dark:border-neutral-800 shadow-xl overflow-hidden will-change-transform";
    ...
    const intervalId = setInterval(nextSection, 3000); // Auto-play every 3s
  ...
  return (
    <section
      ref={containerRef}
      className={cn(
        "relative w-full h-full flex justify-center items-center overflow-hidden bg-transparent",
        className,
      )}
      {...props}
    >
