...
type ActivePopover = "more" | "emoji" | "attach" | "link" | "ai" | "schedule" | null;
...
export const ComposeEmailCard: FC<ComposeEmailCardProps> = ({ data, onSend, onClose }) => {
  ...
  const insertAISuggestion = (text: string) => {
    if (bodyRef.current) {
      bodyRef.current.innerText = (bodyRef.current.innerText || "").trimEnd() + "\n\n" + text;
    }
    setActivePopover(null);
  };
  ...
  const handleFakeAttach = () => {
    const names = ["proposal.pdf", "design-v2.png", "notes.docx", "report.xlsx"];
    const random = names[Math.floor(Math.random() * names.length)];
    if (!attachedFiles.includes(random)) setAttachedFiles((p) => [...p, random]);
    setActivePopover(null);
  };
  ...
  return (
    ...
    >
        ...
        <div className="space-y-2 px-4 pt-4 pb-2 sm:px-8 sm:pt-6">
                    ...
                    {[
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <p className="truncate text-xs font-medium">{acc.name}</p>
          ...
          <div className="flex items-start border-b border-gray-100 py-2 text-[13px] dark:border-zinc-800">
            ...
            <div className="mt-2.5 ml-2 flex gap-2 text-[10px] font-medium text-gray-400 sm:ml-4 sm:gap-3 sm:text-[11px]">
              <button className="transition-colors hover:text-[#6366F1]">CC</button>
              <button className="transition-colors hover:text-[#6366F1]">BCC</button>
            </div>
          </div>

          {/* Subject Section */}
          <div className="flex items-center gap-2 border-b border-gray-100 py-2 sm:gap-4 dark:border-zinc-800">
            <span className="w-12 text-[13px] text-gray-400 sm:w-14">Subject</span>
            <input
              title="subject"
              type="text"
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              ...
              {attachedFiles.map((fname) => (
                ...
                >
                  ...
                  <button
                    onClick={() => setAttachedFiles((p) => p.filter((f) => f !== fname))}
      ...
      <div className="flex flex-none flex-col justify-between gap-4 border-gray-100 bg-[#F5F5F7] px-4 py-3 sm:flex-row sm:items-center sm:px-6 sm:py-4 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <div ref={popoverRef} className="relative flex items-center gap-0.5 text-gray-400 sm:gap-1">
          {/* More */}
          ...
          <div className="flex flex-shrink-0 items-center gap-2">
            ...
            <div className="relative">
              ...
              >
                ...
                {!scheduledTime && <span className="hidden sm:inline">Schedule</span>}
              </button>
              <AnimatePresence>
